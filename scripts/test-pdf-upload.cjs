/**
 * Test script for PDF upload flow (CommonJS version)
 * Run with: node scripts/test-pdf-upload.cjs
 */
...
const PDF_PATH = '/tmp/simple-test.pdf'
...
async function main() {
  ...
  try {
    ...
    await sql`
      ...
      VALUES (${docId}, ${BUSINESS_ID}, 'PDF Test Document', 'pdf-test.pdf', 'active', 'test-script')
    `
    ...
    console.log(`     Uploaded ${(pdfBuffer.length / 1024 / 1024).toFixed(2)} MB`)
    ...
    let attempts = 0
    const maxAttempts = 90  // 90 * 2 = 180 seconds max (3 min for large PDF)
